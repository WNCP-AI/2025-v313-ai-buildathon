...
const HISTORY_STATUSES: BookingRow["status"][] = ["completed", "cancelled"]

function isHistoryStatus(value: string): value is BookingRow["status"] {
  return (HISTORY_STATUSES as string[]).includes(value)
}
...
export default async function OrderHistoryPage({
  ...
  // Base query for history statuses
  let query = supabase
    .from("bookings")
    .select(
      "id, status, scheduled_at, pickup_address, dropoff_address, special_instructions, price_total, consumer_id, provider_id, listing:listing_id(id, title), payment_status",
    )
  ...
  if (statusParam && isHistoryStatus(statusParam)) {
  ...
  const allHistoryStatuses: BookingRow["status"][] = HISTORY_STATUSES
              ...
              <CardContent>
                <form className="grid grid-cols-1 md:grid-cols-5 gap-3" method="GET">
                  ...
                  <div className="md:col-span-5 flex gap-2">
                    ...
                    <Link href="/orders/history" className="inline-flex items-center">
                ...
                {(filtered ?? []).map((b) => (
                  <Card key={b.id}>
                    ...
                    <CardContent className="text-sm space-y-1">
                      ...
                      {b.payment_status && (
                        <div className="text-xs text-gray-600">Payment: {b.payment_status}</div>
                      )}
                    </CardContent>
                  </Card>
                ))}
